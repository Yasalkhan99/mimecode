...
async function main() {
  ...
  const updates: Array<{ id: string; categoryId: string; name: string; categoryName: string }> = [];
  ...
  for (const store of storesData || []) {
    ...
    if (categoryId) {
      ...
      categorizedCount++;
      console.log(`✅ "${store.name}" → ${categoryName}`);
    } else {
      uncategorizedCount++;
      console.log(`❌ "${store.name}" - No matching category found`);
    }
  }
